...
java -jar server/build/libs/server-0.1.5.jar --orchestrator.workspace=/path/to/project
```
...
- `run.cmd`(또는 `gradlew.bat :server:bootRun -PskipWeb`)로 실행하고 http://localhost:47120 을 연다. 빌드한 jar는 `java -jar server\build\libs\server-0.1.5.jar`로 띄운다.
