...
마지막 갱신: 2026-09-18 (v0.1.3 릴리스 후) · 가중 완성도 **약 97%** · 릴리스: v0.1.0 (첫 릴리스, Windows 앱 zip 첨부; 테스트 빌드는 Actions 산출물로)
...
| 9 | Windows 네이티브 지원 | 6 | 100% | .cmd 셈 실행·PATHEXT·설치 위치 탐색·정션 폴백·run.cmd, 데스크톱 앱을 실제 Windows에서 실행·로그인·폴더 선택 확인, 경쟁 모드 1-3-3-1 실제 실행 성공(후보 3 채택·적용, 2026-09-18) |
...
- [x] 실제 Windows에서 경쟁 모드 확인: 빈 폴더에 "준비하기" 후 1-3-3-1로 index.html 랜딩 페이지 생성, 4분 56초, 후보 3개 중 검증자가 후보 3 채택·자동 적용, worktree 정리 확인 (git 신원 기본값 수정 후)
