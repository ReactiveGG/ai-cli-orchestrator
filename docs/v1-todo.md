...
마지막 갱신: 2026-09-17 (브라우저 QA 후) · 가중 완성도 **약 87%** (가중 평균을 다시 계산함. 이전 93%는 표의 수치와 맞지 않았음)
...
| 4 | 서버 (Job 큐·SSE·저장) | 10 | 88% | SSE 하트비트(15초)로 프록시 뒤 끊김 감지·재연결 확인. 로그 정리·이력 관리 미흡 |
| 5 | 웹 UI | 12 | 95% | Chrome 헤드리스로 라이트/다크·1280/400px QA 완료, 오류·로딩·빈 상태·삭제 확인·알림 권한 흐름 정리. 실기기 Edge 확인만 남음 |
...
| 11 | 테스트·CI | 5 | 85% | GitHub Actions(Java: Ubuntu+Windows, 웹: 테스트+빌드), vitest 9개. 스텁 e2e만 남음 |
...
- [ ] SSE 재연결 시 `after=seq`로 이어받기 검증 (지금은 재연결 때 스냅샷+전체 로그를 다시 받음)
