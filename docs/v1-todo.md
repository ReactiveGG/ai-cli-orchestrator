...
마지막 갱신: 2026-09-17 (Claude CLI 실연동 검증 후) · 가중 완성도 **약 64%**
...
| 6 | 에이전트별 모델 선택 | 6 | 80% | 칩마다 모델·에포트 지정 → `--model`/`--effort` 전달. 실모델 확인만 남음 |
...
- [ ] 실제 claude로 `--model sonnet --effort low` 조합이 반영되는지 1회 확인 (result.modelUsage 로 검증)
