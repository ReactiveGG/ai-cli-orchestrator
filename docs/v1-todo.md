...
마지막 갱신: 2026-09-17 (Claude CLI 실연동 검증 후) · 가중 완성도 **약 59%**
...
| 6 | 에이전트별 모델 선택 | 6 | 0% | 도구(claude/codex) 단위까지만 |
...
### 6. 에이전트별 모델 선택
- [ ] 에이전트 스펙에 모델명(예: opus/sonnet) 추가, `claude --model` 전달
- [ ] 구성 화면 모델 블록에 모델명 선택
- [ ] YAML 형식 확장 (`models: [claude:opus, claude:sonnet]` 등)
