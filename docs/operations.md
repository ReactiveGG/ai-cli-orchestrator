...
| 단일 jar | `./gradlew :server:bootJar -PskipWeb` → `java -jar server/build/libs/server-0.1.6.jar` | 다른 PC에 복사해 실행. jar 하나에 서버와 웹 UI(`web/dist`)가 들어 있다(약 25MB) |
...
java -jar server-0.1.6.jar --orchestrator.workspace=C:\dev\my-service
```
...
| 공용 PC라 Claude 로그인을 남기고 싶지 않음 | 대시보드 Claude 타일의 "로그아웃"(확인 창) → `claude auth logout`. 이 로그인은 CLI 것이라 그 PC의 터미널 claude도 함께 해제된다. 앱 종료만으로는 해제되지 않는다 |
