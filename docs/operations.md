...
| 대시보드 Claude 상태 "로그인 필요", 또는 작업이 곧바로 실패하며 "Claude CLI 로그인이 필요합니다" | 서버가 1분마다 `claude auth status`로 확인한다. 터미널에서 `claude`를 실행해 /login(또는 `claude auth login`)한다. 서버는 로그인 화면을 띄울 수 없다. 로그인 후 1분 안에 상태가 바뀐다 |
